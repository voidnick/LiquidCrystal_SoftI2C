...
{
  public:
    LiquidCrystal_SoftI2C(uint8_t lcd_Addr,uint8_t lcd_cols,uint8_t lcd_rows);
  ...
  #ifdef _SLOW_SOFT
    #ifdef _PULLUP
      SlowSoftI2CMaster _ssi2cm = SlowSoftI2CMaster(_SDA, _SCL, _PULLUP);
    #else
      SlowSoftI2CMaster _ssi2cm = SlowSoftI2CMaster(_SDA, _SCL);
    #endif
  #endif
};
#endif
...
LiquidCrystal_SoftI2C::LiquidCrystal_SoftI2C(uint8_t lcd_Addr,uint8_t lcd_cols,uint8_t lcd_rows)
{
  _Addr = lcd_Addr;
  _cols = lcd_cols;
  _rows = lcd_rows;
...
void LiquidCrystal_SoftI2C::expanderWrite(uint8_t _data)
{
  ...
  i2c_stop();
#else
  _ssi2cm.i2c_start(_Addr<<1 + I2C_WRITE);
  _ssi2cm.i2c_write(((int)(_data) | _backlightval));
  _ssi2cm.i2c_stop();
#endif
}
